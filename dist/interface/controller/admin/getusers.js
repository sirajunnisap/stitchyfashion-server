...
const getAllUsers = async (req, res) => {
    try {
        const allUsers = await (0, getUsers_1.getUsers)(userRepository)();
        if (!allUsers) {
...
const getAllPaymentedUsers = async (req, res) => {
    try {
        const allUsers = await (0, getUsers_1.getPaymentedUsers)(userRepository)();
        if (!allUsers) {
            throw new errorHandle_1.AppError("something went wrong", 400);
        }
        res.status(200).json(allUsers);
...
const blockUser = async (req, res) => {
    try {
        ...
        const blockedUser = await (0, getUsers_1.isBlockUser)(userRepository)(userId, action);
        if (blockedUser === null)
...
const searchUsers = async (req, res) => {
    try {
        ...
        const result = await (0, getUsers_1.searchUsecase)(userRepository)(searchQuery, sortCriteria);
